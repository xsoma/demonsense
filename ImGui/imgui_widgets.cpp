...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_act = ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, 1.f);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, 1.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y + 3), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 13), ImColor(165, 0, 0), 12.f);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 15, check_bb.Min.y), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 16.f), ImColor(240, 32, 65), 12.f);
    }
    else
    {
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y + 3), ImVec2(check_bb.Min.x + 30, check_bb.Min.y + 13), ImColor(40, 40, 50), 12.f);
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Min.x + 15, check_bb.Min.y + 16.f), ImColor(230, 230, 230), 12.f);
    }
    if (label_size.x > 0.0f) RenderText(check_bb.Min + ImVec2(36.f, 0.f), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec4 arrow_act = ImVec4(126 / 255.f, 131 / 255.f, 219 / 255.f, 1.f);
...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max, const char* format)
{
    if (format == NULL)
        format = "%.0f deg";
    float v_deg = (*v_rad) * 300.0f / (2*IM_PI);
    bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, format, 1.0f);
    *v_rad = v_deg * (2*IM_PI) / 300.0f;
    return value_changed;
}
