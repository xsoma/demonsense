...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ BETA ] ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ TornunitY] "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ ALPHA ] ")); //-V807
#endif
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0C BETA \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("[ \x0C TornunitY \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("[ \x0C ALPHA \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
	}
}