...
#if RELEASE
#if BETA
#define VERSION crypt_str("TornunitY | ")
#else
#define VERSION crypt_str("TornunitY | ")
#endif
#else
...
void misc::watermark()
{
	...
	const auto col_accent = Color(186, 189, 255); // Watermark line accent color
	...
	render::get().gradient(width - 10 - box_width, 10, box_width, 2, col_accent, col_accent ,GRADIENT_HORIZONTAL);
	//render::get().rect_filled(width - 10 - box_width, 11, box_width, 18, col_background);
	render::get().text(fonts[NAME], width - 10 - box_width + 5, 20, col_text, HFONT_CENTERED_Y, watermark.c_str());
...
void misc::ChatSpamer()
{
	...
	{ 
		crypt_str("BEST RAGE SOFTWARE - TornunitY"),
		crypt_str("TornunitY - PLAY WITH US"),
		crypt_str("GET GOOD. GET TornunitY"),
	...
	if (GetTickCount() - lastspammed > 800)
	{
		...
		std::string msg = crypt_str("say ") + chatspam[rand() % 3];
...
void misc::spectators_list()
{
	...
	for (auto i = 0; i < spectators.size(); i++)
	{
		...
		render::get().text(fonts[NAME], width - x, g_cfg.menu.watermark ? y + 30 : y + 6, col_text, HFONT_CENTERED_Y, spectators.at(i).c_str());
		//render::get().gradient(width - x, g_cfg.menu.watermark ? y + 30 : y, 6, col_accent, col_accent, GRADIENT_HORIZONTAL);
